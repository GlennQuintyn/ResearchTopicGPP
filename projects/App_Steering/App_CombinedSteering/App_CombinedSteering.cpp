...
void App_CombinedSteering::Start()
{
	...
	m_pDrunkWanderer->SetWanderOffset(0);//set offset to 0
	m_pDrunkWanderer->SetNewPointDelay(0.02f);
	m_pBlendedSteering = new BlendedSteering({ { m_pSeek,.5f }, { m_pDrunkWanderer,.5f } });//implicit vetor of weighted behavior 
	//std::vector<BlendedSteering::WeightedBehavior>{BlendedSteering::WeightedBehavior{ m_pSeek,.5f }}; explict version
	...
	m_pEvade = new Evade();
	m_pSoberWanderer = new Wander();
	m_pPrioritySteering = new PrioritySteering({ {m_pEvade},{m_pSoberWanderer} });
	...
	m_pSoberAgent->SetBodyColor({ 0,1,1 });
