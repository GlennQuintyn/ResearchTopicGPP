...
namespace Elite
{
	...
	{
		// remove connections from this pNode
		m_Connections[idx].clear();

		// remove connections from other nodes to this pNode
	...
	template<class T_NodeType, class T_ConnectionType>
	inline void IGraph<T_NodeType, T_ConnectionType>::Render() const
	{
		for (auto node : m_Nodes)
		{
			...
			DEBUGRENDERER2D->DrawSolidCircle(GetNodePos(node), GetNodeRadius(node), { 0,0 }, GetNodeColor(node), -0.9f);
			const auto stringOffset = Vector2{ -0.5f, 1.f };
			DEBUGRENDERER2D->DrawString(GetNodePos(node) + stringOffset, to_string(node->GetIndex()).c_str());
		}
		...
		for (auto connectionList : m_Connections)
		{
			for (auto connection : connectionList)
			{
				auto posTo = GetNode(connection->GetTo())->GetPosition();
				auto posFrom = GetNode(connection->GetFrom())->GetPosition();
				auto center = posTo + (posFrom - posTo) / 2;

				DEBUGRENDERER2D->DrawSegment(posTo, posFrom, GetConnectionColor(connection), 0.f);

				std::stringstream ss;
				ss << std::fixed << std::setprecision(1) << connection->GetCost();
				DEBUGRENDERER2D->DrawString(center, (ss.str()).c_str());
			}
		}
	}
	...
	{
		return GraphNode::DEFAULT_NODE_COLOR;
	}
	...
	{
		return GraphNode::DEFAULT_NODE_RADIUS;
	}
